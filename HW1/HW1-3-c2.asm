...
.text
    ...
    lw $2, R1($0)   # Load R1[Tx] into $2
    lw $3, (4+R1)($0)   # Load R1[Ty] into $3
    lw $4, (8+R1)($0)  # Load R1[Bx] into $4
    lw $5, (12+R1)($0) # Load R1[By] into $5

    # Load R2 values into registers
	la $1, R2
    lw $6, 0($1)   # Load R2[Tx] into $6
    lw $7, 4($1)   # Load R2[Ty] into $7
    lw $8, 8($1)   # Load R2[Bx] into $8
    lw $9, 12($1)  # Load R2[By] into $9
	...
	move $19, $18 #Make a copy of the intersection area in $19
